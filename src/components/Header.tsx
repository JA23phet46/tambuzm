  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-16 z-50 bg-[#fbf9f8]/95 backdrop-blur-md border-b border-[#e4e2e2] flex items-center justify-between px-4 md:px-10">
      ...
      <nav className="hidden md:flex items-center gap-6">
        ...
        <button
          onClick={() => onNavigate(isLoggedIn ? (isAdmin || userRole === UserRole.OWNER ? 'owner-dashboard' : 'seeker-dashboard') : 'login')}
          className={`font-semibold text-sm transition-colors py-2 px-3 rounded-lg ${
            currentPage === 'seeker-dashboard' || currentPage === 'owner-dashboard' ? 'text-[#b52330] font-bold' : 'text-[#5a403f] hover:bg-[#f0eded]'
          }`}
        >
          Dashboard
        </button>
        ...
        {isLoggedIn ? (
          <>
            <div className="flex items-center gap-3">
              ...
              <div 
                onClick={() => onNavigate(isAdmin || userRole === UserRole.OWNER ? 'owner-dashboard' : 'seeker-dashboard')}
